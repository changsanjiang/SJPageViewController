...
open class SJPageViewController: UIViewController {
    ...
    private var modeForHeader: SJPageViewController.HeaderMode = .tracking
    private var heightForHeaderBounds: CGFloat = 0
    private var heightForHeaderPinToVisibleBounds: CGFloat = 0
    ...
    @objc private func _reload() {
        ...
        if let headerView = headerView, self.heightForHeaderBounds == 0 {
            ...
            assert(height != 0, "The headerView height can't be 0!")
...
extension SJPageViewController: SJPageCollectionViewDelegate {
    ...
    open override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if context == &kContentOffset, let childScrollView = object as? UIScrollView {
            ...
            if childScrollView == self.currentVisibleViewController?.sj_pageItem?.scrollView, let headerView = headerView {
                ...
                var headerScrollProgress = abs(y - offset) / (heightForHeaderBounds - heightForHeaderPinToVisibleBounds)
                ...
                self.delegate?.pageViewController(self, headerViewScrollProgressDidChange: headerScrollProgress)
            }
        }
    }
    ...
    private func _callScrollInRange() {
        ...
        if left >= 0 && right < self.numberOfViewControllers {
            ...
            self.delegate?.pageViewController(self, didScrollIn: .init(location: left, length: right), distanceProgress: progress)
        }
    }
