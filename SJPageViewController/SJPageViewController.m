...
@interface SJPageViewController ()<UICollectionViewDataSource, SJPageCollectionViewDelegate, UICollectionViewDelegateFlowLayout, SJPageViewControllerItemCellDelegate> {
    ...
    BOOL _isResponse_progressForHeaderScrollOffsetDidChange;
}
...
- (void)setDelegate:(nullable id<SJPageViewControllerDelegate>)delegate {
    ...
    if ( delegate != nil ) {
        ...
        _isResponse_progressForHeaderScrollOffsetDidChange = [delegate respondsToSelector:@selector(pageViewController:progressForHeaderScrollOffsetDidChange:)];
    }
}
...
- (void)_childScrollViewContentOffsetDidChange:(UIScrollView *)childScrollView change:(nullable NSDictionary<NSKeyValueChangeKey,id> *)change {
    ...
    [self _updateBottomInsetForChildScrollView:childScrollView];

    // 同步 child scrollView 的 contentOffset
    for ( UIViewController *vc in self.viewControllers.allValues ) {
        SJPageItem *pageItem = vc.sj_pageItem;
        if ( ![childScrollView sj_locked] && childScrollView == pageItem.scrollView ) {
    ...
    if ( childScrollView == self.currentVisibleViewController.sj_pageItem.scrollView ) {
        ...
        CGFloat topPin = offset - _heightForHeaderBounds + _heightForHeaderPinToVisibleBounds;
        CGFloat y = frame.origin.y;
        // 向上移动
        if ( newValue >= oldValue ) {
            if ( y < topPin ) y = topPin;
        }
        // 向下移动
        else {
            y += newValue - oldValue;
        ...
        switch ( _modeForHeader ) {
            ...
            case SJPageViewControllerHeaderModeAspectFill: {
                CGFloat value = (-offset - _heightForHeaderBounds);
                if ( offset <= -_heightForHeaderBounds ) {
                    y = offset;
                }
                else {
                ...
                frame.origin.x = -value * 0.5;
                frame.size.width = self.view.bounds.size.width + value;
                frame.size.height = _heightForHeaderBounds + value;
                frame.origin.y = y;
            }
                break;
        }
        ...
        CGFloat indictor = -_heightForHeaderBounds;
        if ( y < -_heightForHeaderBounds ) indictor = y;
        childScrollView.scrollIndicatorInsets = UIEdgeInsetsMake(-indictor, 0, 0, 0);
        
        if ( _isResponse_progressForHeaderScrollOffsetDidChange ) {
            CGFloat progress = (-offset - _heightForHeaderPinToVisibleBounds) / (_heightForHeaderBounds - _heightForHeaderPinToVisibleBounds);
            if ( progress < 0 ) progress = 0;
            else if ( progress > 1 ) progress = 1;
            [self.delegate pageViewController:self progressForHeaderScrollOffsetDidChange:progress];
        }
    }
}
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    return _reload ? self.dataSource.numberOfViewControllers : 0;
}
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(SJPageViewControllerItemCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ( _hasHeader ) {
        ...
        if ( ![item.scrollView sj_locked] && !CGPointEqualToPoint(item.scrollView.contentOffset, contentOffset) ) {
...
- (BOOL)collectionView:(UICollectionView *)collectionView gestureRecognizer:(UIPanGestureRecognizer *)gestureRecognizer shouldRecognizeSimultaneouslyWithGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer {
    ...
    if ( gestureRecognizer == collectionView.panGestureRecognizer && self.dataSource.numberOfViewControllers != 0 ) {
        if ( _hasHeader ) {
        ...
        if ( CGRectContainsPoint(leftEdgeRect, location) ) {
            ...
            if ( [otherGestureRecognizer isKindOfClass:UIPanGestureRecognizer.class] ) {
                CGPoint translate = [gestureRecognizer translationInView:collectionView];
                if ( translate.x < 0 && translate.y == 0 && self.focusedIndex != self.dataSource.numberOfViewControllers - 1 ) {
                    otherGestureRecognizer.state = UIGestureRecognizerStateCancelled;
                    return YES;
                }
            }
...
- (void)_callScrollInRange {
    ...
    if ( left >= 0 && right < self.dataSource.numberOfViewControllers ) {
...
- (void)_insertHeaderViewForRootViewController {
    if ( _hasHeader ) {
        ...
        CGFloat lastItemOffset = ( self.dataSource.numberOfViewControllers - 1 ) * self.collectionView.bounds.size.width;
...
- (BOOL)_isSafeIndex:(NSInteger)index {
    return index < self.dataSource.numberOfViewControllers && index >= 0;
}
...
- (nullable UIView *)_viewForHeader {
    if ( [self.dataSource respondsToSelector:@selector(viewForHeaderWithPageViewController:)] ) {
        return [self.dataSource viewForHeaderWithPageViewController:self];
    }
    return nil;
}
...
- (void)_updateBottomInsetForChildScrollView:(UIScrollView *)childScrollView {
